<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:76:"E:\wamp64\www\Backstage\public/../application/admin\view\category\index.html";i:1530098397;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<script type="text/javascript">
    ...
    layui.use(['table', 'form', 'layer', 'vip_table'], function () {
        ...
        var tableIns = table.render({
            ...
            cellMinWidth: 80,  
            id: 'dataCheck',
            url: "<?php echo url('Category/listCategory'); ?>",
            method: 'get',
            page: true,
        ...
        $('#btn-add').on('click', function () {
            layer.open({
                ...
                maxmin: true,
                area: ['40%', '80%'],
                content: "<?php echo url('Category/addCategory'); ?>",
                end: function(){
        ...
        $('#btn-delete-all').on('click',function(){
            ...
            layer.confirm('真的批量删除用户？', function(index){
                    $.ajax({
                        type: 'post',
                        url: "<?php echo url('Category/batchDel'); ?>",
                        data:{
                            'ids':ids
                        },
                        dataType: 'json',
        ...
        table.on('tool(table)', function(obj){
            ...
            if(layEvent == 'edit'){
                layer.open({
                    ...
                    maxmin: true,
                    area: ['40%', '80%'],
                    content:"<?php echo url('Category/editCategory'); ?>?id="+data.id
                });
            }
            ...
            if(layEvent === 'del'){ 
                layer.confirm('真的删除该栏目？', function(index){
                    $.ajax({
                        type: 'get',
                        url: "<?php echo url('Category/delcategory'); ?>?id="+data.id,
                        dataType: 'json',
                        success: function(data){
                            if(data.status == 0){
